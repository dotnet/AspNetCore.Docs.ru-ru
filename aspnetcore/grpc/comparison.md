...
Все платформы gRPC предоставляют поддержку первого класса для создания кода. Базовым файлом для разработки gRPC является [файл с расширением `.proto`](https://developers.google.com/protocol-buffers/docs/proto3), который определяет контракт служб и сообщений gRPC. Из этого файла платформы gRPC создают базовый класс службы, сообщения и полный клиент.
