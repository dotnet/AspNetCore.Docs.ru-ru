...
## <a name="changes-to-no-locblazor-app-routing-logic-in-501"></a>Изменения в Blazor логике маршрутизации приложений в 5.0.1
...
Если `PreferExactMatches` для задано значение `@true` , то сопоставление маршрутов предпочитает поиск точных совпадений по подстановочным знакам.

> [!IMPORTANT]
> Все приложения должны явно задать `PreferExactMatches` значение `@true` .
>
> Возможность задать значение `PreferExactMatches` `@false` или оставить его неопределенным, что по умолчанию `false` имеет значение в языке C#, *только для обратной совместимости*.
>
...
## <a name="update-no-locblazor-webassembly-and-no-locblazor-server-projects"></a>Обновление Blazor WebAssembly и Blazor Server проекты
...
### <a name="standalone-no-locblazor-webassembly-app-with-azure-active-directory-aad-b2c"></a>Автономное Blazor WebAssembly приложение с Azure Active Directory (AAD) B2C
...
### <a name="hosted-no-locblazor-webassembly-app-with-azure-active-directory-aad-or-aad-b2c"></a>Размещенное Blazor WebAssembly приложение с Azure Active Directory (AAD) или AAD B2C
...
## <a name="update-a-no-locblazor-progressive-web-application-pwa"></a>Обновление Blazor последовательного веб-приложения (PWA)
