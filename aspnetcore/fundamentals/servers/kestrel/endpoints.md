...
Вместо использования параметров `Path` и `Password` для узла сертификата можно указать сертификат с помощью полей хранилища сертификатов. Например, сертификат из раздела `Certificates` > `Default` можно указать следующим образом:
